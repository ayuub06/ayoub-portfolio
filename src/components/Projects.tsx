...
const gradients: Record<string, string> = {
  '01': 'linear-gradient(135deg, #0e4f61 0%, #1e1040 100%)',
  '02': 'linear-gradient(135deg, #0a2d5e 0%, #0e4f61 100%)',
  '03': 'linear-gradient(135deg, #0a3d2e 0%, #0e4f61 100%)',
  '04': 'linear-gradient(135deg, #2d0a5e 0%, #5e0a3d 100%)',
};
    ...
    <section id="projects" ref={ref} className="relative py-32 px-6">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {PROJECTS.map((project: { id: string; title: string; description: string; tags: string[]; image: string; liveUrl: string; githubUrl: string; status: string }, i: number) => (
            <motion.div key={project.id} initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.1 }}
              ...
              <div className="relative overflow-hidden" style={{ height: 220, background: gradients[project.id] }}>
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between items-center">
                  ...
                  {(() => { const s = statusStyle[project.status] ?? statusStyle['Private']; return <span style={{ fontSize: 11, fontWeight: 600, background: s.bg, color: s.color, padding: '3px 10px', borderRadius: 20, border: `1px solid ${s.color}30` }}>{project.status}</span>; })()}
                </div>
              </div>
              <div style={{ padding: '24px 28px' }}>
                <h3 className="group-hover:text-cyan-400" style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 20, color: '#f0f2f8', marginBottom: 10, transition: 'color 0.2s' }}>{project.title}</h3>
