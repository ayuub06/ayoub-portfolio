      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {certs.map((cert, i) => (
            ...
            >
              <div style={{ position: 'relative', height: 140, background: CARD_COLORS[i % CARD_COLORS.length], overflow: 'hidden' }}>
                <Image src={cert.imageUrl} alt={cert.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
              ...
              <div style={{ padding: '16px 18px' }}>
                <div style={{ marginBottom: 8 }}>
                  <h4 style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 13, color: '#f0f2f8', lineHeight: 1.3 }}>{cert.name}</h4>
                  <p style={{ fontSize: 11, color: 'rgba(148,163,184,0.6)', marginTop: 3 }}>{cert.issuer}</p>
                </div>
                {cert.credentialUrl && (
                  <a
          ...
          >
            ...
            >
              <div style={{ position: 'relative', height: 400 }}>
                <Image src={certs[selected].imageUrl} alt={certs[selected].name} fill className="object-contain" style={{ padding: 20 }} />
              </div>
              <div style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
                <div>
                  <h3 style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 16, color: '#f0f2f8' }}>{certs[selected].name}</h3>
                  <p style={{ fontSize: 13, color: 'rgba(148,163,184,0.6)', marginTop: 4 }}>{certs[selected].issuer}</p>
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                  {certs[selected].credentialUrl && (
                    <a href={certs[selected].credentialUrl} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ fontSize: 13, padding: '8px 18px' }}>
                      <span>Credential</span>
                    </a>
                  )}
                  <button
                    onClick={() => setSelected(null)}
                    style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', cursor: 'pointer', fontSize: 20 }}
                  >
                    ×
                  </button>
                </div>
